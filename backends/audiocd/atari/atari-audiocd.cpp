...
AtariAudioCDManager::AtariAudioCDManager()
	...
	debug(1, "AtariAudioCDManager: detected %d MetaDOS CD-ROM drive(s)", _numDrives);
}
...
bool AtariAudioCDManager::openCD(int drive) {
	...
	debug(1, "AtariAudioCDManager: opened CD drive %c:", letter);
	return true;
}
...
int AtariAudioCDManager::ioctl(int command, void *arg) const {
	...
	if (ret < 0)
		debug(2, "AtariAudioCDManager: ioctl(0x%x) failed: %s", command, strerror(-ret));
	return (int)ret;
}
